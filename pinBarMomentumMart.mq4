...
// copying the last bar time to the element New_Time[0]
   ...
   if(Doji(P_CANDLE)){ //specific price target range (20pips) above high
   
      float tp = (Ask+TARGET_PIPS*Point);
      OrderSend(Symbol(),OP_BUY,LOT,Ask,0,0,tp,0,MAGIC,0,clrAliceBlue);
      isNewBar = false;
   
   }else if (Doji(N_CANDLE)){ //specific price target range below low
   
      float tp = (Bid-TARGET_PIPS*Point);
   ...
   void martingale(){
      ...
      if(OrderProfit()<-(OrderLots()*TARGET_PIPS*GRID_SIZE)){ //negative target pips
        
         double PRICE = (OrderType()==OP_BUY?Bid:Ask);
         
         if(OrderSend(Symbol(),OrderType(),OrderLots(),PRICE,0,0,0,0,MAGIC,0,clrBlack)){
            
         OrderSelect(0,SELECT_BY_POS);
        float p1 = OrderOpenPrice();
         OrderSelect((OrdersTotal()-1),SELECT_BY_POS);
        float p2 = OrderOpenPrice();
         
        float tp = (p1+p2)*(OrderType()==OP_BUY?0.50005:0.49995); 
